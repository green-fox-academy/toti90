...
test('max of three: forth', function (t: any): any {
  t.equal(maxOfThree(0, -4, 3), 3);
  t.end();
});
...
test('median: five', function (t: any): any {
  t.equal(median([1, 2, 3, 4, 5, 8]), 3.5);
  t.end();
});
...
test('translate: lago', function (t: any): any {
  t.equal(translate('lagapus'), 'lavagavapuvus');
  t.end();
});

test('translate: haha', function (t: any): any {
  t.equal(translate('haha'), 'havahava');
  t.end();
});